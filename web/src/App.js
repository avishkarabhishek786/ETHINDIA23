...
//import lighthouseEncrypt from "./lighthouseEncrypt";
//import lighthouseDecrypt from "./lighthouseDecrypt";

function App() {
    let chantAndTradeContract = "0x80528EC739aA136326AdA40a6903633d3C0fB1AD"; //zkEVM
    ...
    useEffect(() => {
        ...
        // const lightHouseDecryptFunc = async() => {
        //     await lighthouseDecrypt("QmSzg2enhiJPqVk5Bo23eJsVGtzkQAnaByvmJ1VTbcQviN","71ea6ed777657ec19df3ef03bdedf008e9a3f3ad981f0da98f273f874769ddbc")
        //   }
        //   lightHouseDecryptFunc()
        const getWETHBalance = async () => {
            const { provider, metamaskProvider, signer, network } = await connect();

            const accounts = await metamaskProvider.listAccounts();
            const bal = await WETHread.balanceOf(accounts[0]);
            const formatBal = ethers.utils.formatEther(String(bal))
            setWethBal(formatBal)
        ...
        const getUSDCBalance = async () => {
            ...
            const accounts = await metamaskProvider.listAccounts();
            const bal = await USDCread.balanceOf(accounts[0]);
            const formatBal = ethers.utils.formatEther(String(bal))
            setUSDCBal(formatBal)
    ...
    const pushMessage = async () => {
        await pushMsg("0x64f18353107c6084f16C31C92862C303d733594f");
    };

    //   const lightHouseEncryptFunc = async() => {
    //     await lighthouseEncrypt({
